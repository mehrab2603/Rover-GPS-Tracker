...
class MapPanel extends JPanel {
    ...
    protected synchronized void paintComponent(Graphics g_) {
        ...
        for(Segment seg : segments) {
            ...
            g.drawLine(convertX(pA.getEasting()), convertY(pA.getNorthing(), h), convertX(pB.getEasting()), convertY(pB.getNorthing(), h));
        }

        for(POI poi : pois) {
            int x = convertX(poi.getEasting());
            int y = convertY(poi.getNorthing(), h);
            double degree = poi.getDirection();
            g.setColor(poi.getColor());
            ...
            if(!Double.isNaN(degree)) {
                ...
                temp.rotate(Math.toRadians(degree), 50, getHeight() - 50);
                temp.fillPolygon(new int[] {40, 50, 60, 50}, new int[] {getHeight() - 40, getHeight() - 60, getHeight() - 40, getHeight() - 45}, 4);
                temp.dispose();
                g.setColor(Color.YELLOW);
                g.drawString("Rover Direction", 10, getHeight()- g.getFontMetrics().getHeight());
            }
        }
    ...
    public synchronized void removePOI(POI poi) {
        ...
        updateMinMaxEastingNorthing();
    }
    
    private synchronized void updateMinMaxEastingNorthing() {
        for(int i = 0; i < pois.size(); i++) {
            double easting = pois.get(i).getEasting();
            if(easting > maxEasting) maxEasting = easting;
            if(easting < minEasting) minEasting = easting;

            double northing = pois.get(i).getNorthing();
